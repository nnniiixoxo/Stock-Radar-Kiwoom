...
class KiwoomClient:
    ...
    def authenticate(self) -> str:
        ...
        if not token:
            raise KiwoomError(
                f"접근토큰 발급 실패: {self._message(data)}"
            )
    ...
    def call(
        ...
        *,
        ...
        if api_id not in self.PATHS:
            raise KiwoomError(
                f"허용되지 않은 조회 API: {api_id}"
            )

        if (
            not self.token
            or time.time() >= self.token_expires_at - 60
        ):
            self.authenticate()
        ...
        for attempt in range(retries + 1):
            try:
                response = self.session.post(
                    self.base_url + self.PATHS[api_id],
                    json=body,
                    headers={
                        "Content-Type":
                ...
                return_code = str(
                    data.get(
                        "return_code",
                        data.get("return-code", "0"),
                    )
                )

                if (
                    not response.ok
                    or return_code not in {"0", "", "None"}
                ):
                    raise KiwoomError(
                        f"{api_id} 조회 실패: "
